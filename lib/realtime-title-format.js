...
function candidatePoolForSignal(signal) {
  const pool = [];
  const pushUnique = (value) => {
    const cleaned = cleanStoryTitle(value);
    if (cleaned && !pool.includes(cleaned)) {
      pool.push(cleaned);
    }
  };

  pushUnique(universalWhyTitle(signal));
  buildRealtimeTitleCandidates(signal).forEach(pushUnique);
  pushUnique(cleanStoryTitle(tightenRealtimeTitle(signal.event_or_subject || "", 12)));
  pushUnique(cleanStoryTitle(shortenWords(extractLeadFact(signal), 12)));
  pushUnique(cleanStoryTitle(signal.title));
  return pool;
}
...
function ensureUniqueRealtimeTitles(signals) {
  ...
  return (signals || []).map((signal) => {
    ...
    const pool = candidatePoolForSignal(signal);
    const chosen =
      pool.find((candidate) => {
      ...
      `${formatRealtimeSignalTitle(signal)}: ${tightenRealtimeTitle(signal.event_or_subject || "", 8)}`.trim();

    used.add(cleanStoryTitle(chosen).toLowerCase());
