...
function hasSupabaseConfig() {
  return Boolean(getSupabaseConfigIssue() === null);
}
...
function getSupabaseConfigIssue() {
  ...
  try {
    ...
    if (!url.hostname.includes("supabase")) {
      return "SUPABASE_URL must be your project URL, like https://PROJECT_ID.supabase.co";
    }
...
async function saveFeedback(input) {
  ...
  if (process.env.VERCEL) {
    throw new Error(`Supabase is not configured for production: ${getSupabaseConfigIssue()}`);
  }
